...
To feed context data to SwiftBar, create a status line hook in Claude Code:
...
(tmpdir / input_data.get("session_id", "unknown").json()).write_text(json.dumps({
    "session_id": input_data.get("session_id", "unknown"),
    "pct": ctx_pct,
    "used_tokens": int(input_data.get("context_window", {}).get("used_tokens", 0)),
    "limit_tokens": int(input_data.get("context_window", {}).get("limit_tokens", 0)),
...
Then in Claude Code settings (`~/.claude/settings.json`), add:
```json
"statusLine": {
  "type": "command",
...
SwiftBar will automatically pick up the context data on its next refresh (every 5 minutes).

### How It Works

SwiftBar reads the context data files and displays:

- **Menu bar** — Shows context warnings (🟡🟠🔴) when active sessions are at 70%+
- **Dropdown** — Full list of sessions from the last 24h with context %, model, and age

**Color coding:**
- 🟢 0–69% — Safe
- 🟡 70–79% — Getting full (shows in menu bar)
- 🟠 80–89% — Nearly full (shows in menu bar)
- 🔴 90%+ — Critical (shows in menu bar)

**Session status:**
- `[active]` — Updated within last hour
- `[idle]` — Last seen 1–24h ago (grayed out with age)
- ⚠️ **SAVE!** — At 85%+ context (need ~10–15k tokens to save files)
